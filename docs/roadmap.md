...
4. **Properes iteracions** (pendent PM): coordenades reals Feature 0; backend de producció fora de Vite. **OCR avançat fora d'abast del MVP actual** — 3 rondes de benchmark han confirmat que Tesseract (WASM, CLI, preprocessing simple) no resol els casos d'escaneig molt brut; qualsevol millora OCR futura requereix canvi d'estratègia (motor nou o preprocessing avançat), no tuning incremental.
