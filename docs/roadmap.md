...
3. **`question-answer-extraction`** (MVP tancat): contracte `domain/…`, servei **`qae-api`** permanent a Docker, **nginx proxy invers** (`/api/…`), demo UI **`/demo/qae`** (un sol port, sense firewall extra). Següent: separar enunciat/resposta, millorar segmentació, ensemble OCR o postprocessat LLM.
