...
3. **`question-answer-extraction`** (pas 3 fet): contracte `domain/question-answer-extraction/question_answer_extraction.schema.ts`, `runQuestionAnswerExtractionFromPdf`, diagnòstic separat; harness `npm run spike:qae` imprimeix `{ result, diagnostic }`. Següent: consum (API/UI) o motors addicionals si cal.
