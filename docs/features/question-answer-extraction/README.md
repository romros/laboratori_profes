...
| **UI dev (pas 5)** | **`npm run dev`** + **`npm run dev:qae-api`**. Ruta **`/demo/qae`**: component **`features/question-answer-extraction/ui/QaeDemoPage.tsx`** — pujada PDF, `fetch` via **`getQaeApiUrlForBrowser()`**, llista d’`items` (`question_id`, `status`, `page_indices`, `answer_text`), errors HTTP visibles, diagnòstic en `<details>`. CORS al servidor QAE per origen `localhost` / `127.0.0.1`. |
