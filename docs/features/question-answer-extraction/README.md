...
| **Spike (pas 1–2)**  | Motor intern: OCR **`cat`**, `dedupeQuestionMarkersByFirstId`, truncament si bloc > ~2800 chars (pàgina / paràgraf), `stripTrailingBoilerplateLines`. Tipus interns: `features/.../types/spikeTypes.ts`. **Detecció tolerant:** 3 nivells regex — estricte (`N.`/`N)`), `Pregunta N`, fallback (`N` + paraula d'enunciat) per OCR brut. |
