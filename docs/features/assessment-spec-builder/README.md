...
- **Context al prompt (2.1b):** `enrichAssessmentSpec` crida `buildEnrichAssessmentSpecPrompt({ specJson, examText, solutionText })`; el cos del missatge usuari inclou **tres blocs** (`ASSESSMENT_SPEC_BASE`, `ENUNCIAT ORIGINAL`, `SOLUCIONARI ORIGINAL`). El pipeline `buildAssessmentSpecWithPedagogicEnrichment` reenvia els **mateixos** `examText` / `solutionText` que `buildAssessmentSpec`. **Prova a CI:** `enrichAssessmentSpecPrompt.test.ts` (sentinels del fixture hospital dins del prompt). **Exemple JSON al repo:** `examples/hospital-pipeline-output.sample.json` i `quick-sample-v1-output.json`. Si crides `enrichAssessmentSpec` sol sense aquests strings, el prompt usa placeholders explícits (no inventa context).
