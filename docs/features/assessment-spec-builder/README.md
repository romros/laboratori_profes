...
- **Context al prompt (2.1b):** `enrichAssessmentSpec` crida `buildEnrichAssessmentSpecPrompt({ specJson, examText, solutionText })`; el cos del missatge usuari inclou **tres blocs** (`ASSESSMENT_SPEC_BASE`, `ENUNCIAT ORIGINAL`, `SOLUCIONARI ORIGINAL`). El pipeline `buildAssessmentSpecWithPedagogicEnrichment` reenvia els **mateixos** `examText` / `solutionText` que `buildAssessmentSpec`. **Prova a CI:** `enrichAssessmentSpecPrompt.test.ts` (sentinels del fixture hospital dins del prompt). **I/O reals al repo (GitHub):** `examples/enrich-pass2-user-payload.txt` (tot el que enviem com a `role=user`) i `examples/enrich-pass2-model-output.json` (JSON que retorna el model abans del merge). Regenerar: `npm run write:enrich-pass2-artifacts -w @profes/frontend`. Pipeline complet post-merge: `quick-sample-v1-output.json`. Si crides `enrichAssessmentSpec` sol sense `examText`/`solutionText`, el prompt usa placeholders explícits.
