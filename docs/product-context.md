...
- **Feature 2 (Assessment Spec Builder, pendent):** converteix materials del professor (enunciat + solucionari, text embegut) en un `AssessmentSpec` estructurat i persistent — la representació oficial del criteri d'avaluació. No toca dades d'alumnes. És el prerequisit de Feature 3.
- **Feature 3 (Answer Evaluation, pendent):** combina les respostes extretes (Feature 0/1) amb l'`AssessmentSpec` (Feature 2) per avaluar per pregunta. Aquí hi ha el diferencial real del producte: el professor obté una proposta de correcció estructurada, no un full en blanc.
