...
export async function buildAssessmentSpec(
    ...
    messages: [
      {
        role: 'system',
        content:
          "Ets un extractor estructurat d'avaluació acadèmica. Respon únicament amb un array JSON vàlid, sense markdown.",
      },
  ...
  const questions = z.array(questionSpecSchema).parse(parsed)
