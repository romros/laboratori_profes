...
async function main(): Promise<void> {
  ...
  for (const r of results) {
    ...
    lines.push('')
    lines.push(`**Baseline OCR (pipeline actual):**`)
    lines.push('```')
    lines.push(r.baseline_ocr.trim())
    lines.push('```')
    lines.push('')
  ...
  lines.push('')
  lines.push('| crop_id | baseline | preA | preB | guany preA | guany preB |')
  lines.push('|---------|----------|------|------|-----------|-----------|')
  for (const r of results) {
  ...
  lines.push('')
  lines.push('| Variant | Crops rescatats (baseline=no → variant=yes) |')
  lines.push('|---------|----------------------------------------------|')
  lines.push('| preA    | X/13 |')
  lines.push('| preB    | X/13 |')
  lines.push('')
  lines.push('### Decisió')
  lines.push('')
  lines.push('> [ ] **Opció A — preprocessament útil:** preX rescata ≥ 3/13 crops difícils')
  lines.push('>     → incorporar com a base de Feature 4, continuar amb Spike B (engines)')
  lines.push('>')
  lines.push('> [ ] **Opció B — preprocessament insuficient:** cap variant rescata prou casos')
